...
namespace RoboZhando.Redis.Serialize
{
    ...
    class ListSerializer : RedisSerializer
    {
        public override async Task<object> ReadAsync(RedisConvert convert, Type objectType, object existingValue, Namespace name)
        {
            ...
            if (!types[0].IsPrimitive && !types[0].IsEnum)
                throw new ArgumentException("List type is invalid. Only primitive Lists are supported.", "value");

            //Get the hashset
            List<string> hashset = null; // await convert.Client.FetchListAsync(name.Build());
