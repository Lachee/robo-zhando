...
namespace RoboZhando.Redis
{
    ...
    public class StackExchangeConnection : IRedisConnection
    {
        ...
        /// <returns></returns>
        public async Task<List<string>> FetchListRange(string key, long start = 0, long end = -1)
        {
            RedisValue[] redisValues = await DatabaseAsync.ListRangeAsync(key, start, end);
            return new List<string>(redisValues.Select(rv => rv.ToString()));
        }
